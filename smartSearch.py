...
def get_move(square,mySquareCount, doTarget, targetDir):
    target, direction = max(((neighbor, direction) for direction, neighbor in enumerate(game_map.neighbors(square))
                             if neighbor.owner != myID),
                            default=(None, None),
                            key=lambda t: heuristic2(t[0]))
    ...
    if not border:
        ...
        if doTarget:
            dir = find_nearest_dense_search(square, targetDir)
...
def find_nearest_dense_search(square, targetCoord):
    ...
    if square.strength<150:
        ...
        if lrDist != 0:
            if lrFlip:
                ...
                if (lrDist) > 0:
                    ...
                    neighbour = game_map.get_target(square, WEST)
                    direc = WEST
            maxStrength = neighbour.production
        else:
            maxStrength = 0
        ...
        if udDist != 0:
            if udFlip:
                if (udDist) < 0:
                    neighbour = game_map.get_target(square, SOUTH)
                    if neighbour.production >= maxStrength:
                        direc = SOUTH
                else:
                    neighbour = game_map.get_target(square, NORTH)
                    if neighbour.production >= maxStrength:
    ...
    neighbour = game_map.get_target(square, direc)
    # if neighbour.strength+square.strength>300:
    #     return STILL
    if neighbour.strength < square.strength and neighbour.owner==0:
        return direc
    elif neighbour.owner == myID and square.strength > square.production * 5:
...
writtenToTarget = False
while True:
    game_map.get_frame()

    if ((target==False and target1==False) or beingAttacked==False):
        mySquareCount = 0
        ...
        if target==False:
            ...
            moves = [get_move(square, mySquareCount, True, newTarget) for square in game_map if square.owner == myID]

        max_prod = -10000
        newX = newTarget.x
        newY = newTarget.y
        for square in game_map:
            if square.x == targetCoord.x and square.y == targetCoord.y and square.owner != 0 and target==False:
                target = True

            if square.owner == 0 and target1==False:
                if int(square.production-square.strength) >= max_prod:

                    # if (newTarget.x == targetCoord.x) and (newTarget.y == targetCoord.y):
                    #     max_prod = int(square.production-square.strength)
                    #     newTarget = square
                    # else:
                    #if game_map.get_distance(square,targetCoord) < game_map.get_distance(newTarget,targetCoord):
                    max_prod = int(square.production-square.strength)
                    newTarget = square
                    ...
                    if newY <= game_map.height:
                        ...
                        newY2 = newY - 15
                    #
                    newTarget2 = namedtuple('NewTarget2', 'x y')
                    newTarget2(newX2, newY2)
                    #
                    if game_map.get_distance(targetCoord, newTarget) < game_map.get_distance(targetCoord, newTarget):
                        newTarget = newTarget
                    else:
                        newTarget = newTarget2
                    #
                    # if target == True:
                    #     if square.x == newTarget.x and square.y == newTarget.y and square.owner != 0 and target1==False:
                    #         target1=True
                    #         logging.debug(str(targetCoord))
                    #         logging.debug(str(newTarget))

        # if writtenToTarget == False:
        #     logging.debug(str(targetCoord) + " " + str(newTarget))
        ...
        moves = [get_move(square,mySquareCount,False, []) for square in game_map if square.owner == myID]
        hlt.send_frame(moves)
